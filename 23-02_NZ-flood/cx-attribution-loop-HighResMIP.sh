...
eval_script="https://climexp.knmi.nl/attribute.cgi?EMAIL=$id&STATION=$model_fnm&WMO=$upload_fnm&assume=$fit_type&begin=$obs_start&biasrt=$return_period&ci=$confint&cov1=$gmst_past&dgt=80&end=$event_year&fit=$distribution&includelast=$include_event&restrain=$restrain&timeseries=gmst&type=attribute&year=$event_year"$chsign
 
curl -s ${data_script}
curl -s ${eval_script} > ${eval_logfile}


#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# RUN MODEL ATTRIBUTION

attr_script="https://climexp.knmi.nl/attribute.cgi?EMAIL=$id&STATION=$model_fnm&WMO=$upload_fnm&assume=$fit_type&biasrt=$return_period&ci=$confint&cov1=$gmst_past&dgt=80&end=$event_year&fit=$distribution&includelast=$include_event&restrain=$restrain&timeseries=gmst&type=attribute&year=$event_year"$chsign

curl -s ${data_script}
curl -s ${attr_script} > ${attr_logfile}
...
proj_script="https://climexp.knmi.nl/attribute.cgi?EMAIL=$id&STATION=$model_fnm&WMO=$upload_fnm&assume=$fit_type&ci=$confint&cov1=$gmst_fut&dgt=80&end=2050&fit=$distribution&includelast=$include_event&restrain=$restrain&timeseries=gmst&type=attribute&xyear=$rv&year=$event_year"$chsign

curl -s ${data_script}
curl -s ${proj_script} > ${proj_logfile}
